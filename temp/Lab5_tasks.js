...
task("erc20:mint", "ERC20 mint")
  ...
  .setAction(
    async function ({token, recipient, count}, {ethers: {getNamedSigner}}, runSuper) {
...
task("erc20:approve", "ERC20 approve")
  ...
  .setAction(
    async function ({token, spender, amount}, {ethers: {getNamedSigner}}, runSuper) {
        const erc20 = await ethers.getContractFactory("MyToken1");
        const slp = erc20.attach(token);
        await (await slp.connect(await getNamedSigner("deployer")).approve(spender, ethers.utils.parseEther(amount))).wait();
    }
);
...
task("router:add-liquidity", "Router add liquidity")
  ...
  .addParam("to", "To")
  .setAction(
    async function ({tokenA, tokenB, tokenADesired, tokenBDesired, tokenAMinimum, tokenBMinimum, to, deadline,}, {ethers: {getNamedSigner}}, runSuper) {
        ...
        await (await router.connect(await getNamedSigner("deployer"))
            .addLiquidity(tokenA, tokenB, ethers.utils.parseEther(tokenADesired), ethers.utils.parseEther(tokenBDesired), ethers.utils.parseEther(tokenAMinimum), ethers.utils.parseEther(tokenBMinimum), to,  futureTime)).wait();
    }
);